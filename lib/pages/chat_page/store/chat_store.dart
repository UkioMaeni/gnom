...
abstract class _ChatStore with Store {
  ...
  addMessage(EChatPageType chatType,String message,XFile? file)async{
    if(message.toLowerCase()=="да"||message.toLowerCase()=="нет"){
      final mess=Message(id: 1,status: "send",text: message,sender: "people",fileBuffer: null);
      ...
      if(requiredComplete[chatType.name]!.required){
        if(message.toLowerCase()=="да"){ 
          FormData formData=FormData();
            ...
            final messBot=Message(id: 1,status: "send",text: "Запрос отправлен, ожидайте",sender: "bot",fileBuffer: null);
            chats[chatType.name]!.add(messBot);
            chats=ObservableMap.of(chats);
            final result= await SubjectsHttp().sendRequest(formData);
            if(result!=null){  
              if(result.isEmpty){
                final messBot=Message(id: 1,status: "send",text: "Запрос требует временени на обработку.следите за уведомлениями",sender: "bot",fileBuffer: null);
                chats[chatType.name]!.add(messBot);
              }else{
                final messBot=Message(id: 1,status: "send",text:"Ответ:\n"+ result,sender: "bot",fileBuffer: null);
                chats[chatType.name]!.add(messBot);
              }
              chats=ObservableMap.of(chats);
              
            }else{
              final messBot=Message(id: 1,status: "send",text: "Произошла ошибка",sender: "bot",fileBuffer: null);
          ...
          final messBot=Message(id: 1,status: "send",text: "Запрос отменен",sender: "bot",fileBuffer: null);
          chats[chatType.name]!.add(messBot);
          requiredComplete[chatType.name]!.required=false;
          requiredComplete[chatType.name]!.message=null;
        }
      }else{
        final messBot=Message(id: 1,status: "send",text: "Задайте сначала вопрос",sender: "bot",fileBuffer: null);
        chats[chatType.name]!.add(messBot);
      }
    }else{
      if(file!=null){
        inspect(file);
          final mess=Message(id: 1,status: "send",text: message,sender: "people",fileBuffer:await file.readAsBytes(),name:file.name);
          chats[chatType.name]!.add(mess);
          final messBot=Message(id: 1,status: "send",text: "Вы действительно хотите отправить данную картинку?",sender: "bot",fileBuffer: null);
          chats[chatType.name]!.add(messBot);
          requiredComplete[chatType.name]!.message=mess;
      }else{
          final mess=Message(id: 1,status: "send",text: message,sender: "people",fileBuffer: null);
          chats[chatType.name]!.add(mess);
          final messBot=Message(id: 1,status: "send",text: "Вы действительно хотите запросить \"${message}\"?",sender: "bot",fileBuffer: null);
          chats[chatType.name]!.add(messBot);
          requiredComplete[chatType.name]!.message=mess;
      }
